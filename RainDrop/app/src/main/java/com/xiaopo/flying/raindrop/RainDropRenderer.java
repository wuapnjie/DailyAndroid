...
public class RainDropRenderer implements GLSurfaceView.Renderer {
  ...
  @Override public void onSurfaceChanged(GL10 gl, int width, int height) {
    ...
    options.textureRadio = 192f / 128f;
  ...
  private void generateTexture() {
    textureBgHandle = TextureUtil.loadTextureFromResource(resources, R.drawable.texture_rain_bg);
    textureFgHandle = TextureUtil.loadTextureFromResource(resources, R.drawable.texture_rain_fg);

    if (options.renderShine) {
      textureShineHandle = TextureUtil.loadTextureFromResource(resources, R.drawable.drop_shine2);
    } else {
      textureShineHandle =
          TextureUtil.loadTextureFromBitmap(Bitmap.createBitmap(2, 2, Bitmap.Config.ARGB_8888),
              true);
    }
  ...
  class Options {
    ...
    private float alphaMultiply = 6f;
